...
try {
    ...
    // 1. Récupérer toutes les candidatures de l'animateur connecté
    $formula = "{ID Candidat} = '{$userId}'";
    ...
    foreach ($applicationsResult['records'] as $application) {
        ...
        // 2. Pour chaque candidature, récupérer les détails du camp associé
        $campRecord = callAirtable('GET', 'Camps', null, $campId);
        if (isset($campRecord['error'])) continue;
        $campFields = $campRecord['fields'];

        // 3. Récupérer les détails de l'organisateur du camp
        ...
        // 4. Trier la candidature dans la bonne catégorie
        if (($appFields['Statut'] ?? 'En attente') === 'Accepté') {
            $accepted[] = $data;
        } else {
            $pending[] = $data;
        }
